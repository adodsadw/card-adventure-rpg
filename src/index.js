...
if(u.pathname==="/api/health")return J({ok:true,version:"1.7.5",serverAuthoritative:true,admin:true});
...
async function catalogSyncStatus(env){
  ...
  return J({ok:missing.length===0,inserted:updated,updated,counts,missing,message:missing.length?"仍有預設資料缺少，請執行 sync-defaults-v1.7.5.sql。":`已補齊 ${updated} 筆預設圖片並修復公告時間欄位。`},missing.length?409:200);
}
...
async function lineDiagnostics(env,req){
 const result={ok:true,version:"1.7.5",origin:new URL(req.url).origin,checks:{lineChannelId:!!env.LINE_CHANNEL_ID,lineChannelSecret:!!env.LINE_CHANNEL_SECRET,d1Binding:!!env.DB,oauthStatesTable:false}};
