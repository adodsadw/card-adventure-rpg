const HEROES = [
  {id:"aria",name:"烈焰劍士・亞莉雅",emoji:"⚔️",element:"火",rarity:"稀有",baseHp:520,baseAtk:88,skill:"烈焰斬",skillEmoji:"🔥",skillDesc:"造成 135% 傷害，並有機會形成暴擊。",c1:"#b8402b",c2:"#ffb34c"},
  {id:"mira",name:"冰霜法師・米菈",emoji:"🧙‍♀️",element:"水",rarity:"史詩",baseHp:390,baseAtk:118,skill:"寒冰爆裂",skillEmoji:"❄️",skillDesc:"造成 150% 魔法傷害。",c1:"#2465a8",c2:"#70dcff"},
  {id:"gorn",name:"鋼鐵守衛・戈恩",emoji:"🛡️",element:"土",rarity:"稀有",baseHp:760,baseAtk:62,skill:"盾牌猛擊",skillEmoji:"💥",skillDesc:"造成傷害並以高生命值保護隊友。",c1:"#4d566d",c2:"#aab4cb"},
  {id:"luna",name:"月光祭司・露娜",emoji:"🌙",element:"光",rarity:"史詩",baseHp:470,baseAtk:92,skill:"月華治癒",skillEmoji:"🌟",skillDesc:"攻擊時有 35% 機率治療生命最低的隊友。",c1:"#634fb5",c2:"#d6beff"},
  {id:"kael",name:"暗影刺客・凱爾",emoji:"🥷",element:"暗",rarity:"傳說",baseHp:410,baseAtk:145,skill:"影襲",skillEmoji:"🌑",skillDesc:"高暴擊率的單體突襲。",c1:"#211c45",c2:"#a050d8"},
  {id:"elwyn",name:"森林弓手・艾爾雯",emoji:"🏹",element:"木",rarity:"稀有",baseHp:430,baseAtk:104,skill:"穿風箭",skillEmoji:"🍃",skillDesc:"精準遠程攻擊，傷害波動較小。",c1:"#21784d",c2:"#94d65c"},
  {id:"sol",name:"聖光騎士・索爾",emoji:"✨",element:"光",rarity:"傳說",baseHp:650,baseAtk:120,skill:"神聖審判",skillEmoji:"☀️",skillDesc:"造成 160% 神聖傷害。",c1:"#aa7b20",c2:"#fff0a0"},
  {id:"nyx",name:"夜語術士・妮克絲",emoji:"🔮",element:"暗",rarity:"史詩",baseHp:420,baseAtk:128,skill:"虛空侵蝕",skillEmoji:"🌀",skillDesc:"造成 145% 傷害並強化暴擊。",c1:"#4b1c6e",c2:"#d84cff"}
];
const EQUIPMENT={
...
const SHOP_ITEMS=[
  {id:"energyPotion",name:"體力藥水",emoji:"⚡",desc:"在背包使用後恢復 10 點體力。",price:600,kind:"item"},
  {id:"potion",name:"治療藥水",emoji:"🧪",desc:"未來戰鬥道具系統使用的恢復藥水。",price:300,kind:"item"},
  {id:"wood",name:"古木碎片",emoji:"🪵",desc:"裝備強化與英雄升階素材。",price:180,kind:"material"},
  {id:"ore",name:"星鐵礦石",emoji:"⛏️",desc:"鍛造武器所需素材。",price:260,kind:"material"},
  {id:"herb",name:"月光藥草",emoji:"🌿",desc:"煉金與補給製作素材。",price:220,kind:"material"},
  {id:"sword",name:"星鐵長劍",emoji:"🗡️",desc:"武器：攻擊力 +22。",price:1800,kind:"equipment"},
  {id:"armor",name:"守衛胸甲",emoji:"🥋",desc:"防具：生命值 +120。",price:2200,kind:"equipment"}
];
const STAGES=[
...
function renderHeroDetail(){const h=HEROES.find(x=>x.id===selectedHeroId);if(!h||!state.owned[h.id])return;const st=heroStats(h.id),o=state.owned[h.id],eq=o.equipment||{},slots=[["weapon","武器"],["armor","防具"],["accessory","飾品"]];heroDetail.innerHTML=`<div class="detail-panel"><div class="detail-main"><div class="hero-art" style="--c1:${h.c1};--c2:${h.c2}"><span>${h.emoji}</span></div><h2>${h.name}</h2><div class="stars">${stars(h.rarity)}　${h.rarity}・${h.element}屬性</div><div class="skill-box"><b>${h.skillEmoji} ${h.skill}</b><p style="margin:8px 0 0;color:var(--muted)">${h.skillDesc}</p></div></div><div><div class="detail-stats"><h3>能力與成長</h3><div class="stat-grid"><div class="stat-box"><small>等級</small><b>Lv.${st.level}</b></div><div class="stat-box"><small>綜合戰力</small><b>${st.atk*10+st.hp}</b></div><div class="stat-box"><small>生命</small><b>${st.hp}</b></div><div class="stat-box"><small>攻擊</small><b>${st.atk}</b></div></div><div class="xp-wrap"><div class="xp-bar"><i style="width:${Math.min(100,o.xp/xpNeed(o.level)*100)}%"></i></div><div class="xp-label"><span>英雄經驗</span><span>${o.xp}/${xpNeed(o.level)}</span></div></div></div><div class="equipment-panel" style="margin-top:14px"><h3>裝備穿戴</h3><div class="equipment-list">${slots.map(([slot,label])=>{const eid=eq[slot],item=eid?EQUIPMENT[eid]:null;return `<div class="equipment-slot"><small>${label}</small><div style="font-size:35px">${item?item.emoji:"＋"}</div><b>${item?item.name:"尚未裝備"}</b><button data-equip-slot="${slot}" data-hero-id="${h.id}">${item?"更換":"選擇裝備"}</button></div>`}).join("")}</div></div></div></div>`}
...
function applyCatalogDefaults(c){
  if(Array.isArray(c.heroes)&&c.heroes.length){
    HEROES.splice(0,HEROES.length,...c.heroes.map(h=>({
      ...
      emoji:h.emoji||"✨",imageUrl:h.image_url||"",baseHp:Number(h.base_hp||300),
  ...
  if(Array.isArray(c.shopProducts)&&c.shopProducts.length){
    SHOP_ITEMS.splice(0,SHOP_ITEMS.length,...c.shopProducts.map(x=>({
      id:x.item_id,name:x.name,emoji:x.emoji||"📦",imageUrl:x.image_url||"",
